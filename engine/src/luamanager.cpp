...
void LuaManager::script_file(std::string file) {
	...
	this->m_ready_functions.emplace_back(ready);
	this->m_update_functions.emplace_back(update);
}

void LuaManager::ready() {
	spdlog::trace("LuaManager::ready()");
	for (auto &func : this->m_ready_functions) {
		auto result = func();
		if (!result.valid()) {
			sol::error error = result;
			spdlog::error("[LUA] {}", error.what());
...
void LuaManager::update() {
	for (auto &func : this->m_update_functions) {
		auto result = func();
		if (!result.valid()) {
			sol::error error = result;
			spdlog::error("[LUA] {}", error.what());
